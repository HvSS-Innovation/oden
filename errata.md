...
| 4 | Auth: config reset | `DELETE /api/config/reset` kräver auth ✅ (`docs/WEB_GUI.md`). | Endpointen finns (`oden/web_server.py`) men saknas i `PROTECTED_ENDPOINTS` → **kräver inte token**. Det finns inga tester som verifierar 401 för `/api/config/reset`. | **Kritisk**: destruktiv operation utan auth (även om GUI lyssnar på localhost). | Lägg till `/api/config/reset` i protected-listan och skapa test som verifierar 401 utan token + 200 med token. |
...
- Alla tester passerar (`218 passed`). Errata-listan är alltså **krav-/dokumentationsdriven**: den visar var implementationen inte följer docs (eller där docs behöver förtydligas innan man kan säga vad som är ”rätt”).
